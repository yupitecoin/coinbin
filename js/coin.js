/*
 ...
 A yupitecoin framework.

 http://github.com/OutCast3k/coinjs or http://yupitecoin.com/coinjs
*/
...
(function () {
	...
	/* other vars */
	coinjs.developer = '1CWHWkTWaq1K5hevimJia3cyinQsrgXUvg'; // yupitecoin

	/* bit(yupitecoin.com) api vars */
	coinjs.host = ('https:'==document.location.protocol?'https://':'http://')+'yupitecoin.com/api/';
	coinjs.uid = '1';
	...
	*/
	...
	coinjs.hd = function(data){
		...
		r.master = function(pass) {
			...
			var I = hasher.getHMAC("yupitecoin seed", "TEXT", "SHA-512", "HEX");
	...
	coinjs.transaction = function() {
		...
		r.broadcast = function(callback, txhex){
			...
			coinjs.ajax(coinjs.host+'?uid='+coinjs.uid+'&key='+coinjs.key+'&setmodule=yupitecoin&request=sendrawtransaction&rawtx='+tx+'&r='+Math.random(), callback, "GET");
		}
		...
		r.transactionHash = function(index, sigHashType) {
			...
			if((clone.ins) && clone.ins[index]){
				...
				/* SIGHASH : For more info on sig hashs see https://en.yupitecoin.it/wiki/OP_CHECKSIG
					and https://yupitecoin.org/en/developer-guide#signature-hash-type */
