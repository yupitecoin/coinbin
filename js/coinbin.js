$(document).ready(function() {
		...
		if(email.match(/[\s\w\d]+@[\s\w\d]+/g)){
			if($("#openPass").val().length>=10){
				if($("#openPass").val()==$("#openPassConfirm").val()){
					...
					qrcode.makeCode("yupitecoin:"+keys.address);
	...
	$("#newKeysBtn").click(function(){
		...
		$("#newyupitecoinAddress").val(coin.address);
	...
	function scannerStart(){
		...
		if(navigator.getUserMedia){
			...
			QCodeDecoder().decodeFromCamera(document.getElementById('videoReader'), function(er,data){
				if(!er){
					var match = data.match(/^yupitecoin\:([13][a-z0-9]{26,33})/i);
	...
	$("#redeemFromBtn").click(function(){
		...
		if(host=='blockr.io_yupitecoinmainnet'){
			listUnspentBlockrio_yupitecoinMainnet(redeem);
	...
	function listUnspentBlockrio_yupitecoinMainnet(redeem){
		$.ajax ({
			type: "POST",
	...
	*/
	...
	function rawSubmitDefault(btn){ 
		...
		$.ajax ({
			type: "POST",
			url: coinjs.host+'?uid='+coinjs.uid+'&key='+coinjs.key+'&setmodule=yupitecoin&request=sendrawtransaction',
	...
	function rawSubmitBlockrio_yupitecoinMainnet(thisbtn){ 
		$(thisbtn).val('Please wait, loading...').attr('disabled',true);
		$.ajax ({
			type: "POST",
	...
	// broadcast transaction via blockr.io (mainnet)
	function rawSubmitChainso_yupitecoinMainnet(thisbtn){ 
		$(thisbtn).val('Please wait, loading...').attr('disabled',true);
		$.ajax ({
			type: "POST",
	...
	function rawSubmitblockcypher_yupitecoinMainnet(thisbtn){ 
		$(thisbtn).val('Please wait, loading...').attr('disabled',true);
		$.ajax ({
			type: "POST",
	...
	$(".qrcodeBtn").click(function(){
		...
		if(ta.length>0){
			...
			qrstr = "yupitecoin:"+$('.address',thisbtn).val();
		}
	...
	$("#coinjs_coin").change(function(){
		...
		if(o[5]=="false"){
			...
			$("#coinjs_broadcast").val("yupitecoin.com");			
		} else {
		...
		if(o[6]=="false"){
			...
			$("#coinjs_utxo").val("yupitecoin.com");
		} else {
			$("#coinjs_utxo").val(o[6]);
	...
	function configureBroadcast(){
		...
		} else if(host=="blockr.io_yupitecoinmainnet"){
			$("#rawSubmitBtn").click(function(){
				rawSubmitBlockrio_yupitecoinMainnet(this);
			});
		} else if(host=="chain.so_yupitecoinmainnet"){
			$("#rawSubmitBtn").click(function(){
				rawSubmitChainso_yupitecoinMainnet(this);
			});
		} else if(host=="chain.so_dogecoin"){
			$("#rawSubmitBtn").click(function(){
				rawSubmitchainso_dogecoin(this);
			});
		} else if(host=="blockcypher_yupitecoinmainnet"){
			$("#rawSubmitBtn").click(function(){
				rawSubmitblockcypher_yupitecoinMainnet(this);
			});
		} else if(host=="cryptoid.info_carboncoin"){
			$("#rawSubmitBtn").click(function(){
